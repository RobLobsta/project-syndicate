...
| Session | What it did |
|---|---|
| 47 | **The biped falls over on a turn, and it is one number in the part table.** `CombatArena.Recipe.BIPED` walked in a straight line perfectly well — 0.6° of settled tilt, nine metres on a demand — and went onto its face the moment it was asked to turn: measured over ten seconds of throttle 0.8 at full right lock, **97.7° of tilt, face down from t=390 and it stays there**. Nothing in the suite had ever held a steering demand on a biped. The cause is doc 05 §13.10's clamp and it is arithmetic: an ankle applies `N × half-extent`, a limb in single support carries the whole Assembly so `N ≈ m·g`, and the tilt that bound holds against its own pendulum reduces to `sin θ_max = L / 2h` — the static-stability condition. **A quadruped never has to satisfy it** (duty 0.62 over four limbs keeps two or three planted); **a biped misses it for `2·duty − 1 = 76%` of every stride.** So `mot.limb.strider.t4`'s 0.60 m foot is **6.8°** of recoverable tilt on two limbs against a gait that produces eight. `mot.limb.broad_foot.t4` ships — the strider with a `1.10 × 0.62 m` polygon, 4100 cost, 5 mounts, everything else identical — and the same run is **13.8° worst, 9.3° at the end, −521° of heading, a banked turn held for the whole window**. It is a second row and not a change to the first because the polygon also bounds §13.4's standing re-plant: giving the shared row the big foot took the quadruped's standing travel 0.21 m → **0.79** against a 0.50 ceiling. Doc 05 §13.10 now caps that half at `GaitSolver.STANDING_STEP_TOLERANCE_M` — station-keeping and geometry were one number and only ever agreed because the project authored one foot. Quadruped byte-identical across the change. Suite 8283 → **8291 checks, the same 7 failures**. |
| 46c | **One Prime Mover per locomotion family, and the rotorcraft flies.** Two movers carried four families between them, so a 3.5 t road car and a 10.5 t tracked hauler ran the same 6400 N·m and no family's physics could be tuned without moving the other three — which is why §3.1.1 held a measured, correct raise for four sessions. [PrimeMoverProfile] gains a `locomotion_mask`, `PlacementValidator` gains `PRIME_MOVER_CHASSIS_MISMATCH`, and three rows are authored: `pmv.turbine.tracked.t3` (13 000 N·m, measured), `pmv.combustion.strider.t3` and `pmv.turboshaft.rotary.t3` (320 PU). The wheeled pair keeps both its sections. `CHASSIS_GROUND_TRANSITIONAL` is retired — the new rule is what finally made a vestigial family bit cost something. Tracked run-up **2.14 → 4.98 m/s** and reverse **−2.53 → −5.80 m**; the walking family is byte-identical across the split, which is what the clone was for. **Doc 05 §12.7**: a rotary Assembly's thrust is bolted to its own hull, so any tilt is positive feedback — measured, the shipped recipe lifted off at T/W 1.47, passed 57° at four seconds and came down inverted at 177°. An attitude hold bounded by `T · sin(cyclic_limit) · lever` — what the swashplate could actually produce — now climbs to 317 m holding **1.99°**. It also fixed the rotary brake (12.40 m/s out of a 6.62 run-up, to 0.590) and the arena's hover droop, which was a P controller settling 1.28 m under its target. Suite **13 → 7 failures**. |
| 46b | **The inertia fixture had no rotor, and the tracked ceiling was measured on the wrong bogie.** `test_inertia_coupling` names three parts and had been building two since session 44: the rotor's authored cell stopped mating, `PlacementValidator.commit`'s assert printed and aborted the call, and `before_all` carried on (`LEARNED_FACTS.md` §1 fact 118). Restoring it closed two of the file's three failures and the engine error that was failing the wrapper on its own. The third was the **frame**: a torque-free tumble cannot be seen in world space, where `ω · L` is pinned by conservation — measured, the axis the body genuinely holds reports the *largest* world excursion (fact 119). Re-framed in the body frame, where the minor axis keeps 5.21 of 6.0 rad/s and the intermediate keeps 1.56. **The tracked family measured and not changed**: §3.1.1's drive-torque ceiling was taken on the 1.90 m short bogie and is void — on the 5.60 m one the worst tilt is **1.41° at every torque up to 26 000 N·m**, and 13 000 gives 7.77 m/s against 6400's 3.34. The pivot is *not* torque-bound: it scales linearly and reaches only 21.4° in seven seconds at 26 000. It is not the authored slew damper either — deleting that entirely moves the pivot 2.68° → 3.56°. It is geometry: a 5.60 m patch over a 2.0 m track gauge is an L/B of **2.8** against the ~1.8 skid steering needs. |
| 46 | **A walking Assembly stands still, and traction control stops helping a machine that is crawling.** Two unrelated-looking complaints with one shape: a control acting where it has nothing to act on. **Doc 05 §13.5** — the placement law answered the hip's ground projection outright at zero cadence, so §13.4's standing re-plants each put the foot *directly under a hip that was already travelling*, which resets the lever and arrests nothing. Every walking Assembly in the project has therefore slid: the shipped biped **9.74 m at 2.74 m/s** in five seconds with no demand of any kind, the quadruped 6.85 m, the melee build 10.37 m. Keeping §13.11's capture point — the one term that is a claim about a pendulum rather than about a stride — takes them to **0.15 m, 0.18 m and 0.23 m**. Two files recorded the consequence and neither could see the cause: `test_ambulatory_drift` measured only *heading*, and a machine sliding dead straight changes none, so it now measures translation too; `test_braking_and_reverse` recorded a brake that took a walker from 2.768 m/s to **3.262** over 15.19 m, and now reads 2.77 to **0.12 in 1.25 m**, plus a reverse that went 0.01 m → 1.29 → **6.83**. **Doc 05 §7.6** — the yaw loop's gate read `linear_velocity.length()`, so a build in the air engaged a bicycle model, and it engaged at 1.5 m/s, a walking pace. It now reads the *horizontal* speed and engages above `YAW_CONTROL_SPEED_FRACTION` of the build's own cap — 7.6 m/s on the reference build, derived from where the contacts stop returning an imposed spin to inside the aid's own deadband. The ladder that produced it also settles §7.6's open question the other way: the aid is a **monotone gain at every speed**, and "the loop no longer earns its keep" was a quarter-throttle measurement generalised. Suite **21 failures → 16**, none new. |
| 45 | **A humanoid biped, walking controls, and a preset per locomotion family.** The biped is drawn as a person — waisted torso, chest wider than the shoulder yoke, head, shoulder joints on the flanks, arms to the hip, feet drawn as §13.10's support polygon — and its hands are empty because an edge mates hilt-up under a hanging hand and reaches the floor. **Doc 05 §13.12**: `steer` stopped feeding a lateral velocity into §13.5's correction and became a heading authority — a rate controller on the body's own yaw inertia, shared among planted limbs, targeting the limb's authored turn rate. Full right now comes round −218.7° in five seconds against +24.6° (a *left* turn) before. **The "inverted" placement sign was never inverted**: one number had two consumers and the velocity error won. **Doc 05 §13.10**: the ankle gain was an absolute 60 000 N·m/rad, an aggregate 1.14× the reference biped's own `m·g·h`, so 940 kg of arms took it under the pendulum and the machine fell over from standing; it is now a ratio, and biped tilt, standing yaw drift and the melee stoop all improved with it. §13.4 gains a third re-plant trigger for a foot anchored above the ground it stands on. **Doc 13 §2.1** gains five family proxies — road chassis, walking torso, arm, rolling contact — and `StarterBlueprint` gains `utility`, `tracked`, `ambulatory`, `biped` and `rotary`, paired against the arena's recipes by `test_family_presets`. The capture harness frames itself from measured bounds and draws a floor. |
...
placement chain, the mass solver and the screen flow. What is left is what the
simulation does with it, and `HANDOFF.md` §3.0 is the whole of it.
...
**Superseded in session 45.** Every figure in that table was taken before doc 05
§13.10's ankle became a multiple of the machine's own `m·g·h`. Re-measured: the
contact phase closes to **3.3 m** and resolves on **104** ticks, the duel gains
ground instead of losing it, and the stoop is **8.0°**. The *readings* below all
...
**Superseded in session 45,** and in the direction nobody expected: with §13.10's
ankle scaled to the machine, standing drift falls to **0.54°** and the two
commanded runs separate by **55.1°** on opposite sides of neutral. The demand has
authority — and it is inverted, which could not be seen while it had none.
...
| `test_traction_control` | the launch floor dropped from the allowance; **the corrective brake's ceiling removed**; the slip cut turned into a subtraction; the deadband stepped over rather than subtracted; the grip clamp dropped; the bicycle model's sign; authority not applied to the brake demand; *(session 46)* the yaw loop's engagement speed going back to an absolute figure, which means "fast" on a 14 m/s hauler and "a crawl" on a 24 m/s road build, and the engagement falling under the bicycle model's own singularity guard on a build with a very low cap |
...
| `test_melee_duel` | *(session 43)* the melee recipe holding at the ground family's twenty-metre stand-off, so an edge with 2.4 m of reach parks seventeen short; the orientation group answering "identity" when asked to carry an Appendage's shoulder onto a hull face; and **§15.4's per-swing impulse applied on every tick of contact** — a fault `test_held_weapon` cannot see, because it freezes the target that would be thrown; *(session 45)* re-measured after doc 05 §13.10's proportional ankle — the walker closes 12.0 m to **3.3** where it managed 12.0 to 11.0, gains ground over thirty metres under fire where it used to lose it, and stoops **8.0°** against 12.8 and an original 29.6. Which of the two contact constants is the sharp one has now flipped twice: the ram is back at 2.69 m/s, so the re-opening discriminates again and `TARGET_PEAK_SPEED_MPS` does not |
...
| `test_part_visuals` | *(session 20; no sweep run against it yet — see below)* doc 13 §2.1's collider mirroring by extent, §9's spawn, I-1 over a **populated** `VisualRoot`, the mesh cache's sharing, and the `part_visual` tag suppressing every mesh while touching no collider. *(session 45)* the extent assertion now defends a stronger property than it was written for: a Core Module is carved into a vehicle rather than mirrored, and the same check says the carving still fills the box a round stops on |
| `test_proxy_mesh` (chassis, arm, contact) | *(session 45)* a chassis proxy drawn narrower than its own collider, which is a hull silhouette smaller than the target rounds stop on; the two chassis families collapsing onto one shape, so a torso is drawn as a car; a tracked hull acquiring a family shape it does not want; an arm whose hand stops short of the palm a held module mates at, leaving every blade floating clear of the fist; and a rolling contact falling back to the mirror, which draws half the registry's wheels as cubes |
| `test_gait_solver` (standing plant, session 46) | the standing plant going back to the hip's ground projection, which is the defect that made every walking Assembly in the project slide; the capture point planted *behind* a drifting machine rather than into the drift, which accelerates what it was meant to catch; and a standing plant that keeps the neutral point or the turn rotation, both of which are properties of a stride the machine is not taking |
| `test_biped_balance` (the turn, session 47) | a biped that stands, walks and reverses correctly and lands on its face on the first turn it is asked for. Every other method in the file drives it in a straight line, and in a straight line nothing was wrong; the assertion that catches it is a tilt and a heading **together**, because a machine lying face down has stopped turning and one spinning on its side has turned a very long way |
| `test_ambulatory_drift` (station, session 46) | a standing walker that holds its heading perfectly and travels 6.85 m doing it. The file is named for drift and measured only the rotational half for six sessions, which is exactly how the translational half survived |
| `test_gait_solver` (ankle, session 45) | the ankle gain going back to an absolute constant, which caps how much a walking Assembly may carry at a mass nothing in the data states; the limb count dividing by zero on a part-built Assembly; and a pendulum height below the ground it stands on producing a negative stiffness |
...
| `test_braking_and_reverse` | *(session 38)* the file that would have caught every one of the session's own regressions: a rotary arrest with §15.4's sign inverted (the disc accelerated the hull and flipped it), a tracked build that pitched past vertical under an unproportioned brake, and a gait that could not be asked to stop because the brake demand and the travel demand fought each other. *(Session 46)* both of the things it recorded as they stood have closed: the walking brake went from **accelerating** the machine — 2.768 m/s in, 3.262 out, over 15.19 m — to 2.77 down to 0.12 in 1.25 m, and the reverse from 0.01 m to 6.83 against a wheeled build's 8.18. Both were doc 05 §13.5's standing plant, and neither was a brake or a reverse defect |
