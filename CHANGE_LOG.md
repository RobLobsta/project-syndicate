...
| Session | What it did |
|---|---|
| 45 | **A Gundam, and the constant that was capping every walking machine.** `CombatArena.Recipe.BIPED` gains two Appendages hanging down its flanks on `str.panel.medium.t2` shoulder brackets, an edge in each hand hilt-up (clearing the ground by 1.22 m), and a `cel.static.standard.t3` backpack that is ballast before it is supply. Landing it exposed **doc 05 §13.10's ankle stiffness as an absolute figure**: the aggregate was 1.14 times the reference biped's own `m·g·h`, so 940 kg of arms took it under the pendulum and the machine pitched over from standing. It is now `ANKLE_STIFFNESS_RATIO · m·g·h / limb_count`, and four unrelated numbers moved with it — biped tilt 1.63° → 0.62°, standing yaw drift 18.1° → 0.54°, melee stoop 12.8° → 8.0°, and a steering demand that separates 55.1° where it used to separate 4.7° (and steers the wrong way, which was invisible while it had no authority). §13.4 gains a third re-plant trigger, because the first plant of a spawn is made against a probe no tick has flushed and a foot anchored 2 m in the air leaves the leg *short* rather than long. Doc 13 §2.1 gains four family proxies — a road chassis carved into a floor pan, a body and a greenhouse; an ambulatory one into waist, chest and head; an arm into pauldron, elbow, forearm and hand; and a rolling contact drawn round whatever it collides as (fact 105 leaves half the wheels as boxes). The capture harness frames itself from each Assembly's measured bounds and draws a floor. |
