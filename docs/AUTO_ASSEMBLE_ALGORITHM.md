...
**Status: PLANNED — `src/assembly/autobuild/` is empty and `tests/generation/` runs nothing.** This document is normative for work that has not been done. Nothing here has been tested against a running implementation, so treat its figures as intent rather than as measurements.
...
**1.** Purpose · **2.** Specification Input · **3.** Archetype Definitions · **4.** Pipeline Overview · **5.** Constraint Model · **6.** Phase Detail · **7.** Objective Function · **8.** Backtracking · **9.** Determinism and Threading · **10.** Garage "Suggest" Mode · **11.** Invariants
<!-- END SECTION INDEX -->
