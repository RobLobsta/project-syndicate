...
**Status: PARTIAL — §9's `VisualDamageController` and §10's repair path are unwritten.** Everything else here is built and exercised by the suite.
...
**1.** Purpose · **2.** Damage Channels · **3.** The Damage Packet · **4.** Kinetic Resolution · **5.** Blast Resolution · **6.** Impact Resolution · **7.** Thermal and Corrosive · **8.** Integrity Bands and Functional Degradation · **9.** Visual Damage States · **10.** Repair · **11.** Determinism and Network Authority · **12.** Performance Budget · **13.** Invariants
<!-- END SECTION INDEX -->
...
**What makes the heat fall is the fire, and this section used not to say.** The hysteresis band is unreachable without a cooling term and none was specified, so an ignited part burned for as long as the match lasted. `THERMAL_COOLING_HU_S = 18.0` is charged to the **burning part only** and rides on §7.3's entry: cooling every part in the match on a timer is precisely the poll Invariant I-4 exists to forbid, and a part below the ignition point that keeps its heat is a part closer to catching fire next time — which is the more interesting of the two readings and costs nothing.

Net of the `2.2 HU/s` the fire's own packets deposit through the formula above, an ignited part falls from 480 to 320 in **about ten seconds** and takes forty points of integrity doing it. A fire is what being hit by an energy weapon costs; it is not a second weapon.

**Only a large part can catch fire, and that is arithmetic rather than intent.** Damage and heat come off the same `raw_amount`, so a part must survive

```
480 / 0.55 · (1 − resist_THERMAL) · (1 − armour_absorption)
```

points of thermal damage before it reaches the threshold at all — 542 on `core.command.compact.t2`. Five shipped parts clear that bar (the four Core Modules and `mot.tracked.short_bogie.t2`, with `mot.limb.strider.t4` and `mot.rotor.coaxial_mid.t3` inside 10%) and the rest are destroyed by the fire they would have caught. Nothing here is wrong; it means **fire is a heavy-part mechanic** on the current table, and a part table that wanted a burning wheel would raise that part's thermal resistance rather than lower the threshold.
...
**The shipped entry is §7.1's fire and it is the only kind.** The sketch above carries a `remaining_s` countdown, which is the right shape for an incendiary round or a burning fuel spill; neither is authored, and a submit path with no producer is dead code that no fault sweep can reach. So an entry lives for as long as its part carries `FLAG_OVERHEATED` — the hysteresis band rather than a duration — and a timed entry is the second kind, to be added with the first thing that produces one. `DotScheduler.ignite()` is the whole of the producer side and `DamageResolver._accumulate_heat` is its only caller, on the transition into the flag and not on every packet that arrives afterwards: one entry per burning part, or a build held in a beam would burn at some multiple of the authored rate.

Three further departures from the sketch, all recorded rather than silent:

- **The resolver is an object, not a global.** §3.1's amendment already says why; the scheduler holds a reference and is inert without one.
- **The order inside an entry is damage, then cooling, then the extinction test.** A THERMAL packet raises the heat it is resolved against, so cooling applied first would test a figure the instalment is about to change and a fire would take an extra instalment to notice it had gone out.
- **`MAX_ENTRIES = 128`,** because Invariant I-12 wants every repeatable reaction bounded. One entry per burning part and a part ignites once, so it is reached only by an Assembly alight from end to end.

`raw_amount` on an instalment is the damage for that instalment and `interval_s` is what it covers, so a scheduler that fell behind delivers the same total as one that did not.
