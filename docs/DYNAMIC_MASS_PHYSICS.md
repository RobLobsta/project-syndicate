...
**Status: PARTIAL — §8's aerodynamics has no consumer and §7.5's power band is unread.** Everything else here is built and exercised by the suite.
...
**1.** The Foundational Decision: One Rigid Body Per Assembly · **2.** Runtime Node Structure · **3.** Mass Properties · **4.** Recomputation Policy · **5.** Centre-of-Mass Shift Under Damage · **6.** Suspension · **7.** Traction · **8.** Aerodynamics · **9.** Dynamic Amplification Factor for Strain · **10.** Stutter Elimination · **11.** Invariants · **12.** Rotor Lift and Tilt · **13.** Ambulatory Locomotion · **14.** Tracked Locomotion · **15.** Control Input · **16.** Where the Contact Is Drawn
<!-- END SECTION INDEX -->
...
**The sign is normative and this section used to be silent about it.** `F_arb` is applied **along the contact normal, positive (upward) at the left probe and negative at the right**. The left probe is the negative-`x` end of the pair, so a positive `F_arb` means the left is the more compressed side — the chassis is low on the left — and the couple lifts it. Applied the other way round the term is a *roll amplifier*: it pushes the loaded side further down, and once the inside contact leaves the ground there is no spring on that side left to oppose it.

That is not hypothetical. The implementation applied it inverted for the life of the project, and the failure mode is not a soft roll but a divergence — measured on the reference build at full lock, roll grew `−1.1°, −2.9°, −4.6°, −7.3°, −11.5°, −18.3°, −28.3°, −41.9°, −57.3°, −72.6°` on successive samples and the Assembly finished inverted, from **3.3 m/s**. With the sign correct the same manoeuvre settles at `−2.4°` with all four contacts loaded. A magnitude test cannot see this; the direction needs its own assertion, and `tests/physics/test_wheeled_drive_cycle.gd` carries it.
...
**"Neither drive nor brake" is read against §15.5's released demand, not against the raw record**, and the difference is a defect a player meets the first time they stop to shoot. §15.5 releases the service demand as the hull comes to a stop going forwards, so a driver standing on the brake with no throttle has an *effective* demand of zero the moment they are stationary or being pushed backwards — and reading "brake" off the raw record then refused the holding brake as well. The result was that holding the brake was strictly worse than holding nothing: measured, a parked build taking twenty rounds of its own recoil travelled **1.15 m** with the key released and **10.49 m** with it held. A reverse throttle still counts as drive, so backing out is unaffected.
...
**The throttle may never be a brake, and the taper alone does not guarantee that.** Below about an eighth of the throttle the drag still exceeds the drive, so a demand to accelerate retards the Assembly. That is what a real driveline does at a cracked throttle and it is indefensible as a control: a player pressing the accelerator and slowing down has learned nothing about the machine except that it is untrustworthy.

So the drag is additionally **capped at the drive it is opposing** whenever the throttle is open:

```
τ_drag ← min(τ_drag, τ_capacity · |throttle|)      when throttle ≠ 0
```

which turns that band into a **coasting zone**: the first sliver of throttle releases the engine braking, and only past the crossover does it add drive. Net torque is monotonic in the demand throughout and never negative for a positive one.

The cap is skipped at a shut throttle and has to be — there is no drive to cap against, and `min(τ_drag, 0)` there would delete the term entirely. That leaves a discontinuity at the instant the throttle leaves zero, and **the model cannot avoid one**: full retardation at rest and none at any positive demand cannot both hold continuously. It is placed where a driver expects it, at the moment they touch the throttle, and it is worth `DRIVELINE_DRAG_FRACTION · τ_capacity` — 1.23 m/s² on the reference build.
