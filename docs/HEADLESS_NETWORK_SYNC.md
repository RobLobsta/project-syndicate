...
**Status: PLANNED — `src/net/` holds one file; there is no server, no snapshot codec and no prediction.** This document is normative for work that has not been done. Nothing here has been tested against a running implementation, so treat its figures as intent rather than as measurements.
